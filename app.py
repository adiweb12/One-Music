...
if not DATABASE_URL:
    # Fallback ONLY for local dev; do not use real creds here
    DATABASE_URL = "postgresql://testbase_a998_user:5tLyYQZMQIGBsX3yi9Ht21iRRvJSJMf9@dpg-d2s3n2jipnbc73e71db0-a/testbase_a998"
...
app.config["SESSION_COOKIE_SECURE"] = True  # requires HTTPS (Render provides)
...
# JWT settings
JWT_ISSUER = os.getenv("JWT_ISSUER", "your-chat-app")
JWT_TTL_MINUTES = int(os.getenv("JWT_TTL_MINUTES", "10080"))  # 7 days
JWT_ALG = "HS256"
...
if allowed_origins:
    ...
    # Temporary open CORS for development; set ALLOWED_ORIGINS in prod
    cors = CORS(app)
...
class User(db.Model):
    ...
    # Deprecated: old token column (kept for backward compat, not used)
    token = db.Column(db.String, unique=True, nullable=True)
    groups = db.Column(MutableList.as_mutable(JSONB), default=list)


class Group(db.Model):
    id = db.Column(db.String, primary_key=True)  # group number/code
    name = db.Column(db.String, nullable=False)
    admin = db.Column(db.String, nullable=False)  # username of admin
    members = db.Column(MutableList.as_mutable(JSONB), default=list)
    messages = db.Column(MutableList.as_mutable(JSONB), default=list)
...
def validate_group_number(g: str) -> bool:
    return bool(g and re.fullmatch(r"[A-Za-z0-9_-]{3,32}", g))


def validate_name(n: str) -> bool:
    return bool(n and 1 <= len(n) <= 50)


def validate_message(m: str) -> bool:
    return bool(m and 1 <= len(m) <= 2000)


def get_india_time_iso() -> str:
...
def read_bearer_token():
    ...
    # Fallback: JSON body field "token" for backward compatibility
    if request.is_json:
...
def add_security_headers(resp):
    # Force HTTPS on browsers
    resp.headers["Strict-Transport-Security"] = "max-age=31536000; includeSubDomains; preload"
    # Basic protections
    resp.headers["X-Content-Type-Options"] = "nosniff"
    resp.headers["X-Frame-Options"] = "DENY"
    resp.headers["Referrer-Policy"] = "strict-origin-when-cross-origin"
    # Minimal CSP (adjust if you serve inline scripts/fonts)
    # Note: connect-src includes * to allow API requests from your frontend origin(s).
...
def index():
    return "The secure server is running!", 200
...
@app.post("/login")
@limiter.limit("5/minute")  # slow brute-force
def login():
    data = request.get_json() or {}
    username = (data.get("username") or "").strip()
    password = data.get("password") or ""
    ...
    # JWT is stateless; we don't store tokens in DB
...
@app.post("/get_messages/<group_number>")
@limiter.limit("120/minute")
